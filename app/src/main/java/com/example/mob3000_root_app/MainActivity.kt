...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            App(navController, loginViewModel)
        }
    }
}

@Composable
fun App(navController: NavHostController, loginViewModel: LoginViewModel = LoginViewModel()) {
    MOB3000_ROOT_APPTheme {
        Surface(
            modifier = Modifier.fillMaxSize(),
            color = MaterialTheme.colorScheme.background
        ) {
            Template(navController = navController, loginViewModel)
        }
    }
}